...
def raised_panel_backdrop(fig, x: float, y: float, w: float, h: float, *,
                          edge: str | None = None,
                          glow: str | None = None,
                          depth: float = 0.006,
                          radius: float = 0.010,
...
def chrome(fig, *, section: str, title: str, subtitle: str,
    ...
    fig.text(0.0335, 0.964, "●", color=C_GOLD, fontsize=7,
             family=FONT_SANS)
    fig.text(0.044, 0.958, section, color=TEXT_DIM, fontsize=9.5,
             fontweight="bold", family=FONT_MONO)
    fig.text(0.030, 0.925, title, color=TEXT_BR, fontsize=20,
    ...
    fig.text(0.030, 0.018, f"{hn}  {score}  {an}",
             color=TEXT_BR, fontsize=9.5, fontweight="bold", family=FONT_MONO)
    fig.text(0.5, 0.018, "MATCH ANALYSIS REPORT",
             ha="center", color=TEXT_DIM, fontsize=8.5, fontweight="bold",
             family=FONT_MONO)
    fig.text(0.970, 0.018, footer_note,
             ha="right", color=TEXT_FAD, fontsize=8.5, family=FONT_MONO)
...
def panel_card(fig, x, y, w, h, *, title: str, accent: str = C_GOLD,
    ...
    fig.add_artist(mpatches.Circle((x + 0.018, dot_y), 0.0035,
                   transform=fig.transFigure, facecolor=accent,
                   edgecolor="none", zorder=0))
    fig.text(x + 0.030, dot_y, title.upper(), ha="left", va="center",
...
def metric_card(fig, x, y, w, h, *, label: str, value, accent: str,
    ...
    ax.text(0.5, 0.58, str(value), ha="center", va="center",
            color=readable_on(accent, BG_PANEL), fontsize=27 if big else 21,
            fontweight="bold", family=FONT_MONO, transform=ax.transAxes)
...
def metric_strip(fig, *, cards, x: float = 0.04, w: float = 0.92,
    ...
    for i, (lbl, val, col) in enumerate(cards):
        metric_card(fig, x + i * (cw + gap), y,
                    cw, h, label=lbl, value=val, accent=col,
                    big=True)
...
def key_insight(fig, x, y, w, h, *, text: str, wrap: int = 42):
    ...
    fig.add_artist(mpatches.Circle((x + 0.018, dot_y), 0.0035,
                   ...
                   edgecolor="none", zorder=0))
    fig.text(x + 0.030, dot_y, "KEY INSIGHT", ha="left", va="center",
             color=TEXT_BR, fontsize=10.5, fontweight="bold",
             family=FONT_MONO, zorder=1)
    ...
    ax.text(0.06, 0.86, body, ha="left", va="top",
            color=TEXT_DIM, fontsize=11, family=FONT_SANS,
...
def themed_pitch(ax, *, attacking_only: bool = False,
                 line_color: str = "#A8B8CA", line_alpha: float = 0.56):
    import matplotlib.pyplot as plt
...
def legend_chips(ax, items, y: float = -0.10, fontsize: float = 10):
    ...
    for lbl, col in items:
        ...
        ax.text(x + 0.022, y, lbl.upper(), ha="left", va="center", color=col,
                fontsize=fontsize, fontweight="bold", family=FONT_MONO,
