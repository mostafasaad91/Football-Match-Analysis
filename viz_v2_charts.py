...
from viz_v2 import (
    chrome, panel_card, metric_strip, key_insight, themed_pitch, raised_panel_backdrop,
    BG_DARK, BG_MID, BG_PANEL, BG_HEADER, BG_PITCH, GRID_COL, GRID_SOFT,
    TEXT_BR, TEXT_MAIN, TEXT_DIM, TEXT_FAD,
    C_HOME, C_AWAY, C_GOLD, shadow, readable_on, readable_team_text, ACCENT_TEXT,
    FONT_SANS, FONT_MONO, panel_header_geom,
)
...
def _vp_extent(extent):
    ...
    return [ymin * VP_W / 100.0, ymax * VP_W / 100.0,
            xmin * VP_L / 100.0, xmax * VP_L / 100.0]


def _draw_vertical_pitch(ax, *, attacking_only: bool = False,
                         line_color: str = "#3A3A3A", line_alpha: float = 0.56):
    ...
    for gy, sign in [(0.0, 1), (VP_L, -1)]:
        if attacking_only and gy == 0.0:
            continue
        ax.plot([(VP_W-pa_w)/2, (VP_W-pa_w)/2, (VP_W+pa_w)/2, (VP_W+pa_w)/2],
                [gy, gy + sign*16.5, gy + sign*16.5, gy], **lc)
    ...
    if not attacking_only:
        ax.add_patch(mpatches.Arc((VP_W/2, 16.5), 18, 18, angle=0,
                                  theta1=180, theta2=360, color=line_color,
                                  lw=0.95, alpha=line_alpha*0.70, zorder=2))
    ax.add_patch(mpatches.Arc((VP_W/2, VP_L-16.5), 18, 18, angle=0,
                              theta1=0, theta2=180, color=line_color,
                              lw=0.95, alpha=line_alpha*0.70, zorder=2))
...
def _draw_vertical_attack_arrow(ax, *, x=-1.3, y0=3.0, y1=16.0):
    ...
    ax.add_patch(mpatches.FancyBboxPatch(
        (x - 2.0, y0 - 2.2), 15.5, (y1 - y0) + 4.4,
        boxstyle="round,pad=0.0,rounding_size=1.6",
        facecolor=BG_DARK, edgecolor="none", alpha=0.62, zorder=29))
    ar = mpatches.FancyArrowPatch((x, y0), (x, y1), arrowstyle="-|>",
                                  mutation_scale=14, linewidth=1.6,
                                  color=XT_ARROW, alpha=0.95, zorder=30)
    ax.add_patch(ar)
    ax.text(x + 2.6, (y0 + y1) / 2, "ATTACK", ha="left", va="center",
            color=XT_ARROW, fontsize=7.5, fontweight="bold", alpha=1.0,
            family=FONT_MONO, zorder=31)


def _draw_vertical_arrow(ax, start, end, *, color=XT_ARROW, lw=2.0,
                         alpha=0.9, mutation_scale=12.5, rad=0.04, zorder=10):
    sx, sy = _vp_point(start[0], start[1])
    ex, ey = _vp_point(end[0], end[1])
    ar = mpatches.FancyArrowPatch((sx, sy), (ex, ey), arrowstyle="-|>",
            ...
            if isinstance(patch, mpatches.Rectangle) and patch.get_transform() == self._ax.transData:
                ...
                if _is_pitch_coord([x, x+w], [y, y+h]):
                    ...
                    new_patch = mpatches.Rectangle(
                        ...
                        zorder=patch.get_zorder(), hatch=patch.get_hatch(),
                        fill=patch.get_fill())
            ...
            if isinstance(patch, mpatches.Circle) and patch.get_transform() == self._ax.transData:
                ...
                if _is_pitch_coord(x, y):
                    ...
                    new_patch = mpatches.Ellipse(
                        (nx, ny), width=2*patch.radius*VP_W/100.0,
                        height=2*patch.radius*VP_L/100.0,
                        facecolor=patch.get_facecolor(), edgecolor=patch.get_edgecolor(),
                        linewidth=patch.get_linewidth(), alpha=patch.get_alpha(),
                        zorder=patch.get_zorder(), fill=patch.get_fill())
                    return self._ax.add_patch(new_patch)
        except Exception:
            pass
...
def _label_candidates(x: float, y: float, idx: int) -> list[tuple[float, float, str, str]]:
...
def _rough_label_box(x: float, y: float, label: str, ha: str, va: str,
                     fontsize: float) -> tuple[float, float, float, float]:
...
def _draw_player_label(ax, p: dict, idx: int, *, fontsize: float = 6.0,
                       zorder: int = 9, taken: list | None = None) -> None:
    ...
    ax.text(
        lx, ly, label,
        ha=ha, va=va, color=TEXT_BR, fontsize=fontsize, fontweight="bold",
        bbox=dict(facecolor=BG_DARK, edgecolor="none", alpha=0.64,
...
def render_xg_flow_v2(hn, an, score, hc, ac, shots_h, shots_a,
    ...
    header = mpatches.FancyBboxPatch(
        (PX, PY + PH - header_h), PW, header_h,
        boxstyle="round,pad=0.0,rounding_size=0.006",
        transform=fig.transFigure, facecolor=BG_HEADER,
        edgecolor=GRID_COL, linewidth=1.0, zorder=1,
    )
    fig.add_artist(header)
    dot_y = PY + PH - header_h / 2
    fig.add_artist(mpatches.Circle((PX + 0.018, dot_y), 0.0035,
                   transform=fig.transFigure, facecolor=C_GOLD,
                   edgecolor="none", zorder=2))
    fig.text(PX + 0.030, dot_y, "XG FLOW", ha="left", va="center",
    ...
    for shots, col in [(shots_h, hc), (shots_a, ac)]:
        ...
        for s in ms:
            cum += s["xG"]
            if s["is_goal"]:
                ax.scatter([s["minute"]], [cum], s=170, marker="*",
                           color=C_GOLD, edgecolor=BG_DARK, lw=1.3, zorder=5)
                ax.annotate(s["player"].split()[-1],
                            xy=(s["minute"], cum),
                            xytext=(0, 10), textcoords="offset points",
                            ha="center", color=TEXT_BR, fontsize=8.5,
                            fontweight="bold", family=FONT_SANS,
                            bbox=dict(facecolor=BG_HEADER, edgecolor=GRID_COL,
    ...
    if best and best[0] > 0:
        band_y = y_max * 0.012
        ax.plot([best[1], best[2]], [band_y, band_y], color=C_GOLD,
                lw=4.5, solid_capstyle="round", alpha=0.95, zorder=4)
        ax.text((best[1] + best[2]) / 2, y_max * 0.045,
    ...
    ax.set_xlabel("MINUTE", color=TEXT_DIM, fontsize=9, fontweight="bold",
                  family=FONT_MONO)
    ax.set_ylabel("CUMULATIVE xG", color=TEXT_DIM, fontsize=9, fontweight="bold",
                  family=FONT_MONO)
    # Inline legend chips
    ax.text(0.015, 0.965, "●", color=hc, fontsize=15,
            transform=ax.transAxes, va="top")
    ax.text(0.038, 0.96, hn.upper(), color=hc, fontsize=10, fontweight="bold",
            family=FONT_MONO, transform=ax.transAxes, va="top")
    ax.text(0.015, 0.895, "●", color=ac, fontsize=15,
            transform=ax.transAxes, va="top")
    ...
    ax2.text(0.05, 0.90, "MIN", color=TEXT_DIM, fontsize=8.5,
             fontweight="bold", family=FONT_MONO, transform=ax2.transAxes, va="center")
    ax2.text(0.20, 0.90, "SCORER", color=TEXT_DIM, fontsize=8.5,
             fontweight="bold", family=FONT_MONO, transform=ax2.transAxes, va="center")
    ax2.text(0.93, 0.90, "xG", ha="right", color=TEXT_DIM, fontsize=8.5,
             fontweight="bold", family=FONT_MONO, transform=ax2.transAxes, va="center")
    ax2.plot([0.04, 0.96], [0.84, 0.84], color=GRID_COL, lw=1.0,
    ...
    if goals:
        ...
        ax2.text(0.5, 0.55, "No goals scored", ha="center", va="center",
                 color=TEXT_FAD, fontsize=10, style="italic",
                 family=FONT_SANS, transform=ax2.transAxes)
...
def render_shot_map_v2(team_name, opp_name, score, team_color, shots):
    ...
    fig.add_artist(mpatches.Circle((PX + 0.018, dot_y), 0.0035,
                   ...
                   edgecolor="none", zorder=2))
    fig.text(PX + 0.030, dot_y, "SHOT MAP", ha="left", va="center",
             color=TEXT_BR, fontsize=10.5, fontweight="bold",
             family=FONT_MONO, zorder=2)
    fig.add_artist(mpatches.FancyBboxPatch(
        (PX, PY), PW, body_h, boxstyle="round,pad=0.0,rounding_size=0.006",
        transform=fig.transFigure, facecolor=BG_MID,
        edgecolor=GRID_COL, linewidth=1.0, zorder=-2))
    ...
    if shots:
        ...
        ax.text(VP_W - 1.0, avg_x * VP_L / 100.0 + 0.6, "AVG SHOT", ha="right",
    ...
    if goals:
        ...
        for i, s in enumerate(gsorted):
            ...
            ax.annotate(
                ...
                ha="left", va="center",
                color=C_GOLD, fontsize=8, fontweight="bold", family=FONT_SANS,
                arrowprops=dict(arrowstyle="-", color=GRID_COL, lw=0.9,
    ...
    for lbl, col, mk in chips:
        fig.text(cx, PY - 0.018, mk, ha="left", va="center", color=col,
                 fontsize=10.5, family=FONT_SANS)
        fig.text(cx + 0.013, PY - 0.018, lbl.upper(), ha="left", va="center",
                 color=TEXT_DIM, fontsize=7.3, fontweight="bold", family=FONT_MONO)
    ...
    ax2 = panel_card(fig, 0.50, 0.50, 0.46, 0.38,
                     title="Top Shot Sources (by xG)", accent=team_color)
    ax2.set_xlim(0, 1); ax2.set_ylim(0, 1)
    ax2.text(0.04, 0.90, "PLAYER", color=TEXT_DIM, fontsize=8.7,
             fontweight="bold", family=FONT_MONO, transform=ax2.transAxes, va="center")
    ax2.text(0.62, 0.90, "SH",  ha="center", color=TEXT_DIM, fontsize=8.7,
             fontweight="bold", family=FONT_MONO, transform=ax2.transAxes, va="center")
    ax2.text(0.78, 0.90, "G",   ha="center", color=TEXT_DIM, fontsize=8.7,
             fontweight="bold", family=FONT_MONO, transform=ax2.transAxes, va="center")
    ax2.text(0.95, 0.90, "xG",  ha="right",  color=TEXT_DIM, fontsize=8.7,
             fontweight="bold", family=FONT_MONO, transform=ax2.transAxes, va="center")
    ax2.plot([0.03, 0.97], [0.84, 0.84], color=GRID_COL, lw=1.0,
             transform=ax2.transAxes)
    if top:
        n = len(top); rh = 0.74 / n
        for i, (player, d) in enumerate(top):
            cy = 0.78 - (i + 0.5) * rh
            if i > 0:
                ax2.plot([0.03, 0.97], [cy + rh/2, cy + rh/2], color=GRID_SOFT,
                         lw=0.8, transform=ax2.transAxes, zorder=1)
            label = (player or "—").split()[-1] if player else "—"
            ax2.text(0.04, cy, label, ha="left", va="center",
                     color=TEXT_BR, fontsize=10.5, fontweight="bold",
                     family=FONT_SANS, transform=ax2.transAxes, zorder=2)
            ax2.text(0.62, cy, str(d["shots"]), ha="center", va="center",
                     color=TEXT_DIM, fontsize=10, family=FONT_MONO,
                     transform=ax2.transAxes, zorder=2)
            ax2.text(0.78, cy, str(d["goals"]),
                     ha="center", va="center",
                     color=C_GOLD if d["goals"] else TEXT_DIM,
                     fontsize=10, fontweight="bold", family=FONT_MONO,
                     transform=ax2.transAxes, zorder=2)
            ax2.text(0.95, cy, f"{d['xG']:.2f}", ha="right", va="center",
                     color=team_color, fontsize=10.5, fontweight="bold",
                     family=FONT_MONO, transform=ax2.transAxes, zorder=2)
    else:
        ax2.text(0.5, 0.4, "No shots recorded", ha="center", va="center",
                 color=TEXT_FAD, fontsize=10, style="italic",
                 family=FONT_SANS, transform=ax2.transAxes)
...
def render_shot_breakdown_v2(hn, an, score, home, away, goals, hc=None, ac=None):
    ...
    ax1 = panel_card(fig, 0.04, 0.49, 0.56, 0.32,
    ...
    ax1.text(0.012, 0.965, "●", color=hc, fontsize=13,
             transform=ax1.transAxes, va="top")
    ax1.text(0.034, 0.965, hn.upper(), color=hc, fontsize=9.5, fontweight="bold",
             family=FONT_MONO, transform=ax1.transAxes, va="top")
    ax1.text(0.012, 0.895, "●", color=ac, fontsize=13,
             transform=ax1.transAxes, va="top")
    ax1.text(0.034, 0.895, an.upper(), color=ac, fontsize=9.5, fontweight="bold",
             family=FONT_MONO, transform=ax1.transAxes, va="top")

    keys   = ["shots", "on_target", "off_target", "blocked", "post"]
    labels = ["Total Shots", "On Target", "Off Target", "Blocked", "Woodwork"]
    n = len(keys)
    pos = np.arange(n); w = 0.34
    ...
    for i, (hv, av) in enumerate(zip(h_vals, a_vals)):
        ax1.text(i - w/2, hv + bar_ymax * 0.018, str(hv), ha="center", va="bottom",
                 color=TEXT_BR, fontsize=10.5, fontweight="bold", family=FONT_MONO)
        ax1.text(i + w/2, av + bar_ymax * 0.018, str(av), ha="center", va="bottom",
                 color=TEXT_BR, fontsize=10.5, fontweight="bold", family=FONT_MONO)
    ax1.set_xticks(pos)
    ax1.set_xticklabels(labels, color=TEXT_DIM, fontsize=9.5, family=FONT_SANS,
                        fontweight="bold")
    ax1.tick_params(axis="x", length=0, pad=10); ax1.set_yticks([])
    ...
    for c, x in cols:
        ax2.text(x, 0.87, c, ha="left" if c != "xG" else "right",
                 va="center", color=TEXT_DIM, fontsize=9, fontweight="bold",
                 family=FONT_MONO, transform=ax2.transAxes)
    ax2.plot([0.03, 0.97], [0.81, 0.81], color=GRID_COL, lw=1.0,
             transform=ax2.transAxes)
    if goals:
        n_g = len(goals)
        row_h = 0.68 / n_g
        for i, g in enumerate(goals):
            ...
            if i > 0:
                ax2.plot([0.03, 0.97], [cy + row_h/2, cy + row_h/2],
                         color=GRID_SOFT, lw=0.8, transform=ax2.transAxes, zorder=1)
            # Minute chip, matching the reference `.row .min` styling.
            ax2.add_patch(mpatches.FancyBboxPatch(
                (0.035, cy - 0.038), 0.06, 0.076,
                boxstyle="round,pad=0.0,rounding_size=0.010",
                facecolor=C_GOLD, alpha=0.10, lw=0,
                transform=ax2.transAxes, zorder=2))
            ax2.text(0.065, cy, mn, ha="center", va="center",
            ...
            ax2.text(0.34, cy, team_nm, ha="left", va="center",
                     color=team_col, fontsize=10, fontweight="bold",
                     family=FONT_MONO, transform=ax2.transAxes, zorder=2)
            type_text, accent = _type_badge_style(gtype, team_col)
            ax2.text(0.49, cy, type_text[:24], ha="left", va="center",
                     color=accent, fontsize=8.4, fontweight="bold",
                     family=FONT_MONO, transform=ax2.transAxes, zorder=3,
                     bbox=dict(boxstyle="round,pad=0.32",
                               facecolor=to_rgba(accent, 0.08),
                               edgecolor=to_rgba(accent, 0.35), linewidth=1.0))
            ax2.text(0.64, cy, assist, ha="left", va="center",
                     color=TEXT_DIM, fontsize=10, family=FONT_SANS,
                     transform=ax2.transAxes, zorder=2)
            ax2.text(0.92, cy, f"{xg:.2f}", ha="right", va="center",
                     color=TEXT_BR, fontsize=11, fontweight="bold",
                     family=FONT_MONO, transform=ax2.transAxes, zorder=2)
    else:
        ax2.text(0.5, 0.45, "No goals scored", ha="center", va="center",
                 color=TEXT_FAD, fontsize=11, style="italic",
                 family=FONT_SANS, transform=ax2.transAxes)
...
def _infer_position_bucket(p: dict, depth_axis: str = "x",
                           direction: int = 1) -> str:
...
def render_pass_network_v2(team_name, opp_name, score, team_color,
    ...
    fig.add_artist(mpatches.Circle((PX + 0.018, dot_y), 0.0035,
                   transform=fig.transFigure, facecolor=team_color,
                   edgecolor="none", zorder=2))
    fig.text(PX + 0.030, dot_y, "PASS NETWORK", ha="left", va="center",
             color=TEXT_BR, fontsize=10.5, fontweight="bold",
             family=FONT_MONO, zorder=2)
    fig.add_artist(mpatches.FancyBboxPatch(
        (PX, PY), PW, body_h, boxstyle="round,pad=0.0,rounding_size=0.006",
    ...
    mid_col = "#A8B8CA"
    drawable_edges = [
        e for e in sorted(edges, key=lambda d: d["count"])
    ...
    for e in drawable_edges:
        ...
        if e["count"] >= strong_cut:
            ...
            z = 2
        pax.plot([p1["x"], p2["x"]], [p1["y"], p2["y"]],
                color=line_col, lw=lw, alpha=alpha,
    ...
    for p1, p2, count, _lw in top_for_labels:
        ...
        pax.text(mx, my, str(count), ha="center", va="center",
                color=TEXT_BR, fontsize=7.2, fontweight="bold",
                family=FONT_MONO,
                bbox=dict(boxstyle="round,pad=0.20",
                          facecolor=BG_DARK, edgecolor=GRID_COL,
                          lw=0.8, alpha=0.92),
                zorder=4)
    ...
    for rank, p in enumerate(players_sorted):
        ...
        pax.scatter([p["x"]], [p["y"]], s=size + 260, color=BG_DARK,
                   alpha=0.92, zorder=5)
        pax.scatter([p["x"]], [p["y"]], s=size + 110, color=ring_color,
                   alpha=0.40 if sub_ring else 0.28, zorder=5)
        pax.scatter([p["x"]], [p["y"]], s=size, color=node_color,
                   edgecolor=TEXT_BR, lw=1.35, alpha=0.98, zorder=6)
        ...
        pax.text(p["x"], min(100, max(0, p["y"] + label_offset)), short,
                ha="center", va=va,
                color=TEXT_BR, fontsize=name_fs, fontweight="bold",
                family=FONT_SANS,
                bbox=dict(boxstyle="round,pad=0.22",
                          facecolor=BG_DARK, edgecolor="none",
                          alpha=0.88), zorder=7)
        pax.text(p["x"], p["y"], str(p["passes"]),
                ha="center", va="center",
                color=BG_DARK, fontsize=7.4, fontweight="bold",
    ...
    ax2.set_xlim(0, 1); ax2.set_ylim(0, 1)
    ax2.text(0.04, 0.90, "PAIR", color=TEXT_DIM, fontsize=8.7,
             fontweight="bold", family=FONT_MONO, transform=ax2.transAxes, va="center")
    ax2.text(0.95, 0.90, "PASSES", ha="right", color=TEXT_DIM, fontsize=8.7,
             fontweight="bold", family=FONT_MONO, transform=ax2.transAxes, va="center")
    ax2.plot([0.03, 0.97], [0.84, 0.84], color=GRID_COL, lw=1.0,
             transform=ax2.transAxes)
    top_edges = sorted(edges, key=lambda e: -e["count"])[:8]
    if top_edges:
        n = len(top_edges); rh = 0.74 / n
        for i, e in enumerate(top_edges):
            ...
            ax2.text(0.04, cy, pair[:23], ha="left", va="center",
                     color=TEXT_BR, fontsize=10.5, fontweight="bold",
                     family=FONT_SANS, transform=ax2.transAxes, zorder=2)
            ax2.text(0.95, cy, str(e["count"]), ha="right", va="center",
                     color=team_color, fontsize=11, fontweight="bold",
                     family=FONT_MONO, transform=ax2.transAxes, zorder=2)
    else:
        ax2.text(0.5, 0.4, "No links recorded", ha="center", va="center",
                 color=TEXT_FAD, fontsize=10, style="italic",
                 family=FONT_SANS, transform=ax2.transAxes)
...
def _themed_pass_pitch_vertical(ax, *, line_alpha: float = 0.52):
    ...
    for y0, sign in [(0, 1), (l, -1)]:
        ...
        ax.plot([(w - ga_w) / 2, (w - ga_w) / 2, (w + ga_w) / 2, (w + ga_w) / 2],
                [y0, y0 + sign * 5.5, y0 + sign * 5.5, y0], **lc)
        ax.scatter([w / 2], [y0 + sign * 11], color=line_grey,
                   s=6, alpha=line_alpha, zorder=2)


def _draw_player_label_vertical(ax, p: dict, idx: int, *, fontsize: float,
                                zorder: int, taken: list | None = None,
                                all_nodes: list | None = None,
                                pitch_w: float = PASS_PITCH_W,
                                pitch_l: float = PASS_PITCH_L,
                                node_radius: float = 3.4,
                                node_radii: dict | None = None) -> None:
    taken = taken if taken is not None else []
    ...
    if ((lx - p["x"]) ** 2 + (ly - p["y"]) ** 2) ** 0.5 > own_r + 2.4:
        ax.plot([p["x"], lx], [p["y"], ly], color="#6A6A6A", lw=0.4,
                alpha=0.5, zorder=zorder - 1, solid_capstyle="round")
    ax.text(lx, ly, label, ha=ha, va=va, color=TEXT_BR,
...
def _draw_pass_network_half(ax, title, players, edges, accent):
    ...
    ax.text(0.02, 0.985, title.upper(), transform=ax.transAxes,
            ha="left", va="top", color=accent, fontsize=10.5,
            fontweight="bold", family=FONT_MONO, zorder=20)
    for idx, p in enumerate(sorted(players, key=lambda d: d.get("passes", 0), reverse=True), start=1):
        p["display_id"] = idx
    _depth_axis = _detect_depth_axis(players)
    _direction = _detect_attack_direction(players, _depth_axis)
    ...
    for i, e in enumerate(drawable_edges):
        ...
        if glow_alpha:
            ax.plot([p1["x"], p2["x"]], [p1["y"], p2["y"]],
                    color=line_col, lw=lw + 4.0, alpha=glow_alpha,
                    solid_capstyle="round", zorder=z)
        ax.plot([p1["x"], p2["x"]], [p1["y"], p2["y"]],
                color=line_col, lw=lw, alpha=alpha,
                solid_capstyle="round", zorder=z)
    ...
    if len(core) >= 3:
        ...
        if len(hull) >= 3:
            ax.add_patch(mpatches.Polygon(
                hull, closed=True, facecolor=accent, edgecolor=accent,
                alpha=0.06, lw=1.0, joinstyle="round", zorder=1))
            ax.add_patch(mpatches.Polygon(
                hull, closed=True, facecolor="none", edgecolor=accent,
                alpha=0.28, lw=1.0, joinstyle="round", zorder=1))
    ...
    if len(by_depth) >= 5:
        ...
        ax.plot([2, PASS_PITCH_W - 2], [dline, dline],
                ls=(0, (6, 4)), color=accent, lw=1.0, alpha=0.45, zorder=1)
        ax.text(PASS_PITCH_W - 2, dline + 0.6, "DEF LINE", ha="right", va="bottom",
                color=accent, fontsize=5.6, fontweight="bold",
                family=FONT_MONO, alpha=0.7, zorder=1)

    # Identify the goalkeeper (positional GK, else the deepest node) so the
    # node can carry a distinct gold edge.
    gk_node = None
    for p in v_players:
        if _infer_position_bucket({"x": p.get("_orig_x", p["y"]),
                                   "position": p.get("position")}) == "gk":
            gk_node = p
            break
    ...
    for rank, p in enumerate(sorted(v_players, key=lambda d: d.get("passes", 0), reverse=True)):
        ...
        ax.scatter([p["x"]], [p["y"]], s=size, color=node_color,
                   marker=marker, edgecolor=edge_c, lw=edge_w, alpha=0.90, zorder=6)
    ...
    ax.add_patch(mpatches.FancyBboxPatch(
        (arrow_x - 1.2, 75.5), 2.6, 22.0,
        boxstyle="round,pad=0.0,rounding_size=1.0",
        facecolor=BG_DARK, edgecolor="none", alpha=0.55, zorder=18))
    ax.annotate("", xy=(arrow_x, 95), xytext=(arrow_x, 79),
...
def render_pass_network_halves_v2(team_name, opp_name, score, team_color,
    ...
    rx = 0.030
    fig.add_artist(mpatches.Circle((rx + 0.006, 0.866), 0.0050,
                   transform=fig.transFigure, facecolor=team_color,
                   edgecolor=TEXT_BR, linewidth=0.8, zorder=5))
    fig.text(rx + 0.018, 0.866, f"{team_name.upper()} (STARTER)", ha="left", va="center",
             color=TEXT_BR, fontsize=8.7, fontweight="bold",
             family=FONT_MONO, zorder=5)
    rx += 0.018 + 0.0072 * len(team_name + " (STARTER)") + 0.030
    sq = 0.0092
    fig.add_artist(mpatches.Rectangle((rx, 0.866 - sq / 2), sq, sq,
                   transform=fig.transFigure, facecolor=team_color,
                   edgecolor=TEXT_BR, linewidth=0.8, zorder=5))
    fig.text(rx + 0.016, 0.866, "SUBSTITUTE", ha="left", va="center",
             color=TEXT_BR, fontsize=8.7, fontweight="bold",
             family=FONT_MONO, zorder=5)
    ...
    for px, pw, ptitle, pcol in panel_specs:
        ...
        header_h = 0.040
        header = mpatches.FancyBboxPatch(
            (px, panel_y + panel_h - header_h), pw, header_h,
            boxstyle="round,pad=0.0,rounding_size=0.006",
            transform=fig.transFigure, facecolor=BG_HEADER,
            edgecolor=GRID_COL, linewidth=1.0, zorder=-1)
        fig.add_artist(header)
        dot_y = panel_y + panel_h - header_h / 2
        fig.text(px + 0.018, dot_y, ptitle.upper(), ha="left", va="center",
                 color=pcol, fontsize=10, fontweight="bold",
                 family=FONT_MONO, zorder=1)
        fig.text(px + pw - 0.018, dot_y, "ATTACK →", ha="right", va="center",
                 color=TEXT_FAD, fontsize=8.5, fontweight="bold",
                 family=FONT_MONO, zorder=1)
    ...
    h2_shown = _draw_pass_network_half(ax2, "", second_half["players"], second_half["edges"], h2_col)
    ...
    fig.text(0.105, 0.172,
             f"1H · {h1_passes} passes · {h1_players} players · "
             f"{h1_shown}/{len(first_half['edges'])} links · number = shirt number",
             color=TEXT_DIM, fontsize=9, fontweight="bold", family=FONT_MONO)
    fig.text(0.560, 0.172,
    ...
    cards = [
        ...
        ("Total Links", f"{len(first_half['edges']) + len(second_half['edges'])}", C_GOLD),
    ]
    metric_strip(fig, cards=cards)
    return fig
...
def render_xt_map_v2(team_name, opp_name, score, team_color, passes):
    ...
    fig.add_artist(mpatches.Circle((PX + 0.018, dot_y), 0.0035,
                   transform=fig.transFigure, facecolor=team_color,
                   edgecolor="none", zorder=2))
    fig.text(PX + 0.030, dot_y, "XT MAP", ha="left", va="center",
             color=TEXT_BR, fontsize=10.5, fontweight="bold",
             family=FONT_MONO, zorder=2)
    fig.add_artist(mpatches.FancyBboxPatch(
        (PX, PY), PW, body_h, boxstyle="round,pad=0.0,rounding_size=0.006",
    ...
    for p in top5_passes:
        _draw_vertical_arrow(ax, (p["x"], p["y"]), (p["end_x"], p["end_y"]),
                             ...
                             mutation_scale=12.5, rad=0.055, zorder=8)
    ...
    ax2 = panel_card(fig, 0.55, 0.50, 0.41, 0.38,
                     title="Top xT Creators", accent=team_color)
    ax2.set_xlim(0, 1); ax2.set_ylim(0, 1)
    ax2.text(0.04, 0.90, "PLAYER", color=TEXT_DIM, fontsize=8.7,
             fontweight="bold", family=FONT_MONO, transform=ax2.transAxes, va="center")
    ax2.text(0.65, 0.90, "PASS", color=TEXT_DIM, fontsize=8.7,
             fontweight="bold", family=FONT_MONO, transform=ax2.transAxes, va="center", ha="center")
    ax2.text(0.95, 0.90, "xT", ha="right", color=TEXT_DIM,
             fontsize=8.7, fontweight="bold", family=FONT_MONO,
    ...
    if top_creators:
        ...
        for i, (nm, d) in enumerate(top_creators):
            ...
            if i > 0:
                ax2.plot([0.03, 0.97], [cy + rh/2, cy + rh/2], color=GRID_SOFT,
                         lw=0.8, transform=ax2.transAxes, zorder=1)
            ax2.text(0.04, cy, (nm or "—").split()[-1],
                     ha="left", va="center",
                     color=TEXT_BR, fontsize=10.5, fontweight="bold",
                     family=FONT_SANS, transform=ax2.transAxes, zorder=2)
            ax2.text(0.65, cy, str(d["n"]), ha="center", va="center",
                     color=TEXT_DIM, fontsize=10, family=FONT_MONO,
                     transform=ax2.transAxes, zorder=2)
            ax2.text(0.95, cy, f"{d['xT']:.3f}", ha="right", va="center",
                     color=C_GOLD, fontsize=10.5, fontweight="bold",
                     family=FONT_MONO, transform=ax2.transAxes, zorder=2)
    else:
        ax2.text(0.5, 0.4, "No positive-xT passes", ha="center", va="center",
                 color=TEXT_FAD, fontsize=10, style="italic",
                 family=FONT_SANS, transform=ax2.transAxes)
    ...
    cards = [
        ...
        ("Top Creator", (top_creators[0][0].split()[-1] if top_creators else "—"), TEXT_BR),
    ]
    metric_strip(fig, cards=cards)
    return fig
...
def _match_colors(info):
    ...
    return (
        _clean_dark_navy(info.get("home_color") or info.get("HOME_COLOR") or _team_color_fallback(hn, C_HOME)),
        _clean_dark_navy(info.get("away_color") or info.get("AWAY_COLOR") or _team_color_fallback(an, C_AWAY)),
    )
...
def make_pass_network_v2(events, info, team_id, team_color):
    ...
    def _build_network(src_evts):
        ...
        succ = src_evts[(src_evts["is_pass"] == True) &
                        (src_evts["outcome"] == "Successful")]
        for i in range(len(succ)):
            ...
            if passer_id is None:
                continue
            later = src_evts[(src_evts.index > curr_idx) &
                             src_evts["player_id"].notna()]
            if later.empty:
                continue
            nxt = later.iloc[0]
            ...
            try:
                ...
                if all(v is not None and not (isinstance(v, float) and np.isnan(v))
                       for v in vals):
                    if ((float(pe_x) - float(nx_)) ** 2 +
                            (float(pe_y) - float(ny_)) ** 2) ** 0.5 > RECV_MAX_DIST:
                        continue
            except Exception:
                pass
    ...
    return render_pass_network_halves_v2(
        team_name, opp_name, str(score), team_color,
...
def render_pitch_overlay_v2(
    *,
    ...
    team_color, draw_overlay, draw_legend=None,
    ...
    fig.add_artist(mpatches.Circle((PX + 0.018, dot_y), 0.0035,
                   transform=fig.transFigure, facecolor=team_color,
                   edgecolor="none", zorder=2))
    fig.text(PX + 0.030, dot_y, section, ha="left", va="center",
             color=TEXT_BR, fontsize=10.5, fontweight="bold",
             family=FONT_MONO, zorder=2)
    fig.add_artist(mpatches.FancyBboxPatch(
        (PX, PY), PW, body_h, boxstyle="round,pad=0.0,rounding_size=0.006",
    ...
    ax = fig.add_axes([PX + 0.035, PY + legend_reserve, PW - 0.07,
                       body_h - legend_reserve - 0.030])
    ...
    for i, (lbl, x) in enumerate(zip(sidebar_headers, xs)):
        ...
        ax2.text(x, 0.90, lbl, ha=ha, va="center",
        ...
        for i, row in enumerate(sidebar_rows):
            ...
            for j, (val, x) in enumerate(zip(row, xs)):
                ...
                ax2.text(x, cy, str(val), ha=ha, va="center",
                         color=col, fontsize=fs, fontweight=fw, family=fam,
                         transform=ax2.transAxes, zorder=2)
    else:
        ax2.text(0.5, 0.4, "No data recorded", ha="center", va="center",
                 color=TEXT_FAD, fontsize=10, style="italic",
                 family=FONT_SANS, transform=ax2.transAxes)
...
def _smooth_density_grid(grid: np.ndarray, radius: int = 4,
                         sigma: float = 1.55) -> np.ndarray:
...
def make_defensive_heatmap_v2(events, info, team_id, team_color):
    ...
    def draw_overlay(ax):
        ...
        grid, _, _ = np.histogram2d(
            ys, xs, bins=(34, 48), range=((0, 100), (0, 100))
        )
        ...
        if np.any(density):
            ...
            alpha[norm < 0.055] = 0
            ax.imshow(
                norm, extent=[0, 100, 0, 100], origin="lower",
                cmap=DEF_HEAT_CMAP, alpha=alpha, interpolation="bilinear",
                zorder=2, aspect="auto",
            )
            levels = [0.28, 0.48, 0.68]
            ax.contour(
                norm, levels=levels, extent=[0, 100, 0, 100],
                origin="lower", colors=[team_color, "#facc15", "#ffffff"],
                linewidths=[0.65, 0.85, 1.05], alpha=0.34, zorder=3,
            )
            ...
            if len(hot_idx):
                for r, c in hot_idx[::max(1, len(hot_idx) // 7)][:7]:
                    ...
                    ax.scatter([hx], [hy], s=70, facecolor="none",
                               edgecolor="#ffffff", lw=0.9, alpha=0.50,
                               zorder=4)
        ...
        for t, col in DEF_TYPE_COLORS.items():
            ...
            px = [p[0] for p in pts]
            py = [p[1] for p in pts]
            ax.scatter(px, py, s=34, facecolor=col, edgecolor="#ffffff",
                       lw=0.35, alpha=0.66, zorder=5)
    ...
    def draw_legend(fig, panel_x, panel_y):
        ...
        for lbl, col in density_items:
            fig.add_artist(plt.Circle((lx, ly), 0.0035, transform=fig.transFigure,
                           facecolor=col, edgecolor=TEXT_BR, lw=0.7, zorder=10))
            fig.text(lx + 0.014, ly, lbl, ha="left", va="center",
                    color=TEXT_DIM, fontsize=8, fontweight="bold",
                    family=FONT_MONO, zorder=10)
        ...
        for t in action_types:
            ...
            fig.add_artist(plt.Circle((ax_x, ay), 0.0028, transform=fig.transFigure,
                           facecolor=col, edgecolor=TEXT_BR, lw=0.5, zorder=10))
            fig.text(ax_x + 0.013, ay, lbl.upper(), ha="left", va="center",
                    color=TEXT_DIM, fontsize=7.5, fontweight="bold",
                    family=FONT_MONO, zorder=10)
...
def make_avg_positions_v2(events, info, team_id, team_color):
    ...
    rows = [(f"#{p['display_id']}",
             ...
             str(p["touches"])) for p in players[:9]]

    if players:
        avg_x = int(np.mean([p["x"] for p in players]))
        width_spread = int(max(p["y"] for p in players) -
                           min(p["y"] for p in players))
        length_spread = int(max(p["x"] for p in players) -
...
def make_high_turnovers_v2(events, info, team_id, team_color):
    ...
    def draw_overlay(ax):
        ...
        ax.axvline(60, color=C_GOLD, lw=1.3, ls=(0, (3, 2)), alpha=0.7,
                  zorder=1)
        ax.text(80, 97, "HIGH ZONE", ha="center", va="top", color=C_GOLD,
                fontsize=7.5, fontweight="bold", family=FONT_MONO, alpha=0.95,
                zorder=2, bbox=dict(boxstyle="round,pad=0.25",
                facecolor=BG_DARK, edgecolor="none", alpha=0.65))
        for x, y, t in points:
            col = DEF_TYPE_COLORS.get(t, team_color)
            ax.scatter([x], [y], s=150, facecolor=col, edgecolor=TEXT_BR,
...
def make_pass_thirds_v2(events, info, team_id, team_color):
    ...
    def draw_overlay(ax):
        ...
        for tx, lbl in [(16.5, "DEF"), (50, "MID"), (83, "ATT")]:
            ax.text(tx, 96.5, lbl, color=TEXT_BR, fontsize=8,
                    fontweight="bold", family=FONT_MONO, ha="center", va="top",
                    zorder=5, bbox=dict(boxstyle="round,pad=0.22",
                    facecolor=BG_DARK, edgecolor=GRID_COL, linewidth=0.8,
                    alpha=0.88))
...
def make_ball_touches_v2(events, info, *,
    ...
    return render_pitch_overlay_v2(
        ...
        team_color=hc, draw_overlay=draw_overlay,
...
def render_bar_compare_v2(*, section, title, subtitle, hn, an, score,
    ...
    ax.set_facecolor(BG_MID)
    ax.text(0.012, 0.965, "●", color=hc, fontsize=13,
            transform=ax.transAxes, va="top")
    ax.text(0.034, 0.965, hn.upper(), color=hc, fontsize=9.5, fontweight="bold",
            family=FONT_MONO, transform=ax.transAxes, va="top")
    ax.text(0.012, 0.895, "●", color=ac, fontsize=13,
            transform=ax.transAxes, va="top")
...
def make_xt_per_minute_v2(events, info):
    ...
    if "xT" not in events.columns:
        ...
        ax.text(0.5, 0.5, "No xT data", ha="center", va="center",
                color=TEXT_DIM, fontsize=14, family=FONT_SANS,
    ...
    fig.add_artist(mpatches.Circle((PX + 0.018, dot_y), 0.0035,
                   transform=fig.transFigure, facecolor=C_GOLD,
                   edgecolor="none", zorder=2))
    fig.text(PX + 0.030, dot_y, "XT PER MINUTE", ha="left", va="center",
             color=TEXT_BR, fontsize=10.5, fontweight="bold",
             family=FONT_MONO, zorder=2)
    ...
    ax.plot(mins, _hv, color=hc, lw=2.0, alpha=0.95, zorder=5,
            solid_capstyle="round")
    ax.plot(mins, _av, color=ac, lw=2.0, alpha=0.95, zorder=5,
            solid_capstyle="round")
...
def make_gk_saves_v2(events, info):
    ...
    for i, (team_id, opp_id, team_name, team_color, x0, w) in enumerate([
        ...
        fig.add_artist(mpatches.Circle((PX + 0.018, dot_y), 0.0035,
                       transform=fig.transFigure, facecolor=team_color,
                       edgecolor="none", zorder=2))
        fig.text(PX + 0.030, dot_y, team_name.upper(), ha="left", va="center",
                 color=TEXT_BR, fontsize=10.5, fontweight="bold",
                 family=FONT_MONO, zorder=2)
        fig.add_artist(mpatches.FancyBboxPatch(
            (PX, PY), PW, PH - header_h,
            boxstyle="round,pad=0.0,rounding_size=0.006",
            transform=fig.transFigure, facecolor=BG_MID,
            edgecolor=GRID_COL, linewidth=1.0, zorder=-2))
        ...
        for _, r in sub.iterrows():
            ...
            if bool(r.get("is_goal")):
                pax.scatter([x], [y], s=80 + xg * 1500, marker="*",
                           color=C_GOLD, edgecolor=team_color,
                           lw=1.5, alpha=0.98, zorder=5)
                n_goal += 1
            elif stype == "SavedShot":
                pax.scatter([x], [y], s=60 + xg * 1300, facecolor="none",
                           edgecolor=team_color, lw=2.0, alpha=0.92,
                           zorder=4)
                n_save += 1
            else:
                pax.scatter([x], [y], s=40 + xg * 1100, facecolor="none",
                           edgecolor=TEXT_FAD, lw=1.0, alpha=0.55,
                           zorder=3)
        # Per-keeper summary line, just below the pitch header.
        fig.text(x0 + w/2, PY + PH - header_h - 0.022,
...
def render_legacy_chart_v2(
    *, section, title, subtitle, hn, an, score, footer_note,
    ...
    insight_text, metric_cards, legacy_box=None,
):
    """
    ...
    if sidebar_rows:
        ...
        for i, row in enumerate(sidebar_rows):
            ...
            for j, (val, x) in enumerate(zip(row, xs)):
                ...
                fam = FONT_SANS if j == 0 else FONT_MONO
                ax2.text(x, cy, str(val), ha=ha, va="center",
                         color=col, fontsize=10.5 if j == 0 else (11 if is_last else 10),
                         fontweight="bold" if (j == 0 or is_last) else "normal",
                         family=fam, transform=ax2.transAxes, zorder=2)
    else:
        ax2.text(0.5, 0.4, "No data recorded", ha="center", va="center",
                 color=TEXT_FAD, fontsize=10, style="italic",
                 family=FONT_SANS, transform=ax2.transAxes)
