...
def draw_player_stats_table(df: pd.DataFrame, team_name: str,
    ...
    for i, (_, r) in enumerate(df.iterrows()):
        ...
        base_color = BG_PANEL if i % 2 == 0 else ("#FFFFFF" if is_light_theme else "#0A0A0A")
        if not is_starter:
...
def _draw_section_divider(pdf, num: str, title: str, subtitle: str,
    ...
    # خط فاصل — تحت الـ subtitle (اللي ممكن يبقى سطرين بعد الـ wrap)
...
def _draw_player_radar_page(pdf, player, pizza_fig, accent, page_no=None,
                            team_name="", role="", commentary=""):
    ...
    # header
    fig.text(0.07, 0.958, "Player Radar", ha="left", va="center",
             color=TEXT_BRIGHT, fontsize=18, fontweight="bold", family="serif")
    fig.text(0.93, 0.958, str(team_name), ha="right", va="center",
             color=accent, fontsize=12, fontweight="bold", family="monospace")
    line_ax = fig.add_axes((0.07, 0.936, 0.86, 0.002))
    line_ax.set_facecolor(accent); line_ax.set_xticks([]); line_ax.set_yticks([])
    for s in line_ax.spines.values():
        s.set_visible(False)
    ...
    if img_ar >= W_in / H_in:
        w_in = W_in; h_in = W_in / img_ar
    else:
        h_in = H_in; w_in = H_in * img_ar
    ...
    ax_t.set_axis_off(); ax_t.set_xlim(0, 1); ax_t.set_ylim(0, 1)
    ax_t.text(0.0, 1.0, "TACTICAL READ", ha="left", va="top", color=accent,
              fontsize=11, fontweight="bold", family="monospace")
    wrapped = "\n\n".join(_tw.fill(p.strip(), width=104)
                          for p in str(commentary).split("\n\n") if p.strip())
    ax_t.text(0.0, 0.90, wrapped, ha="left", va="top", color=TEXT_MAIN,
              fontsize=9.3, family="serif", linespacing=1.45)
...
def _draw_closing_page(pdf, info, events=None, ppda=None):
    ...
    try:
        ...
        bgimg = fig.add_axes([0.146, 0.0, 0.708, 1.0]); bgimg.set_axis_off()
        bgimg.imshow(img, alpha=0.32, zorder=0, aspect="auto")
    except Exception:
        pass
    veil = fig.add_axes([0, 0, 1, 1]); veil.set_axis_off()
    veil.set_xlim(0, 1); veil.set_ylim(0, 1)
    veil.add_patch(mpatches.Rectangle((0, 0), 1, 1, facecolor=BG_DARK, alpha=0.42, lw=0, zorder=1))
    veil.add_patch(mpatches.Rectangle((0.02, 0.026), 0.96, 0.948, facecolor="none",
                   edgecolor=C_GOLD, lw=1.0, alpha=0.4, zorder=2))
    for (x, y, dx, dy) in [(0.035, 0.958, 1, -1), (0.965, 0.958, -1, -1),
                           (0.035, 0.042, 1, 1), (0.965, 0.042, -1, 1)]:
    ...
    fig.text(0.5, 0.57, "MATCH ANALYSIS REPORT", ha="center", color=TEXT_BRIGHT,
             fontsize=26, fontweight="bold", family="monospace", zorder=5)
    aln = fig.add_axes([0.36, 0.545, 0.28, 0.0018]); aln.set_facecolor(C_GOLD)
    aln.set_xticks([]); aln.set_yticks([])
    for s in aln.spines.values():
        s.set_visible(False)
    if recap:
        fig.text(0.5, 0.50, recap, ha="center", color=C_GOLD, fontsize=12.5,
    ...
    fig.text(0.5, 0.36, "Created by Mostafa Saad", ha="center", color=TEXT_BRIGHT,
             fontsize=13, family="serif", zorder=5)
    fig.text(0.5, 0.30,
             "Data: WhoScored / Opta   ·   Built with Python & Matplotlib   ·   "
             "Analysis by Mostafa Saad",
             ha="center", color=TEXT_FADED, fontsize=8.5, family="monospace", zorder=5)
...
def _page_rail(fig, accent, *, label="", page_no=None):
    ...
    rail.set_xticks([]); rail.set_yticks([])
    for s in rail.spines.values():
        s.set_visible(False)
    fig.text(0.035, 0.028, "MATCH ANALYSIS REPORT", color=TEXT_FADED,
             fontsize=7.5, fontweight="bold", family="monospace")
    if label:
        fig.text(0.5, 0.028, label.upper(), ha="center", color=TEXT_FADED,
                 fontsize=7.5, fontweight="bold", family="monospace")
    if page_no is not None:
        fig.text(0.965, 0.028, f"{page_no:02d}", ha="right", color=TEXT_FADED,
                 fontsize=7.5, fontweight="bold", family="monospace")
...
def _match_kpis(events, info, ppda):
    ...
    hid, aid = info.get("home_id"), info.get("away_id")
    out = {"xg": [0.0, 0.0], "xt": [0.0, 0.0], "shots": [0, 0], "sot": [0, 0],
           "big": [0, 0], "poss": [50.0, 50.0], "goals": [0, 0],
           "ppda": [None, None]}
    try:
        ...
        if "is_pass" in events.columns:
            ...
            if ph + pa:
                out["poss"] = [round(100 * ph / (ph + pa), 1), round(100 * pa / (ph + pa), 1)]
        out["ppda"] = [ppda.get("home", {}).get("ppda"), ppda.get("away", {}).get("ppda")]
    except Exception:
        pass
    return out
...
def _draw_executive_summary_page(pdf, info, events, ppda, goals_df):
    ...
    fig.text(0.06, 0.945, "EXECUTIVE SUMMARY", color=C_GOLD, fontsize=12,
             fontweight="bold", family="monospace")
    fig.text(0.06, 0.90, f"{hn}  {score}  {an}", color=TEXT_BRIGHT,
    ...
    if abs(xh - xa) < 0.15:
        ...
        verdict = (f"{better} carried the stronger chance quality "
    ...
    fig.text(0.06, 0.80, "VERDICT", color=TEXT_DIM, fontsize=10, fontweight="bold",
             family="monospace")
    fig.text(0.06, 0.755, _wrap_text_simple(verdict + over, 82),
             color=TEXT_BRIGHT, fontsize=13.5, va="top", linespacing=1.35)
    ...
    cards = [
        ...
        ("xT", _fmt(k['xt'][0], dec=True), _fmt(k['xt'][1], dec=True), k['xt'][0], k['xt'][1], False),
        ("Big Chances", str(k['big'][0]), str(k['big'][1]), k['big'][0], k['big'][1], False),
        ("Shots", str(k['shots'][0]), str(k['shots'][1]), k['shots'][0], k['shots'][1], False),
        ("Possession", _fmt(k['poss'][0], pct=True), _fmt(k['poss'][1], pct=True), k['poss'][0], k['poss'][1], False),
        ("PPDA", _fmt(k['ppda'][0], dec=True), _fmt(k['ppda'][1], dec=True), k['ppda'][0], k['ppda'][1], True),
    ]
    cw, gap = 0.135, 0.018
    x0 = 0.06
    cy, ch = 0.50, 0.16
    for i, (lbl, hv, av, hh, aa, lower_better) in enumerate(cards):
        cx = x0 + i * (cw + gap)
        ax = fig.add_axes([cx, cy, cw, ch]); ax.set_axis_off()
        ax.set_xlim(0, 1); ax.set_ylim(0, 1)
        ax.add_patch(mpatches.FancyBboxPatch((0.02, 0.02), 0.96, 0.96,
                     boxstyle="round,pad=0.0,rounding_size=0.05",
                     facecolor=BG_PANEL, edgecolor=GRID_COL, lw=1.0))
        ax.text(0.5, 0.86, lbl.upper(), ha="center", color=TEXT_DIM,
        ...
        try:
            ...
            h_win = None
        ax.text(0.29, 0.44, hv, ha="center", color=C_GOLD if h_win is True else TEXT_BRIGHT,
                fontsize=14, fontweight="bold", family="monospace")
        ax.text(0.71, 0.44, av, ha="center", color=C_GOLD if h_win is False else TEXT_BRIGHT,
                fontsize=14, fontweight="bold", family="monospace")
        ax.text(0.29, 0.14, hn[:9], ha="center", color=C_HOME, fontsize=6.8, fontweight="bold")
        ax.text(0.71, 0.14, an[:9], ha="center", color=C_AWAY, fontsize=6.8, fontweight="bold")

    # ── Key moments ────────────────────────────────────────────────────
    moments = _key_moments(events, info, goals_df)
    fig.text(0.06, 0.40, "KEY MOMENTS", color=TEXT_DIM, fontsize=10,
             fontweight="bold", family="monospace")
    my = 0.355
    if moments:
        for mn, scorer, team, tag in moments[:6]:
            col = C_HOME if team == hn else (C_AWAY if team == an else TEXT_BRIGHT)
            fig.text(0.06, my, f"{mn}'", color=C_GOLD, fontsize=11,
                     fontweight="bold", family="monospace")
        ...
        fig.text(0.06, my, "No goals in normal/extra time.", color=TEXT_DIM,
                 fontsize=10, style="italic")

    # ── The story (momentum) ───────────────────────────────────────────
    poss_leader = hn if k['poss'][0] > k['poss'][1] else an
    xt_leader = hn if k['xt'][0] >= k['xt'][1] else an
    story = (f"{poss_leader} saw more of the ball, while {xt_leader} moved it "
             f"into threatening areas more effectively. The following pages break "
             f"down how each side created and denied danger.")
    fig.text(0.62, 0.40, "THE STORY", color=TEXT_DIM, fontsize=10,
             fontweight="bold", family="monospace")
    fig.text(0.62, 0.355, _wrap_text_simple(story, 44), color=TEXT_MAIN,
             fontsize=11, va="top", linespacing=1.55)
...
def _draw_verdict_page(pdf, info, events, ppda, goals_df, page_no=None):
    ...
    fig.text(0.06, 0.935, "THE VERDICT", color=C_GOLD, fontsize=12,
             fontweight="bold", family="monospace")
    fig.text(0.06, 0.875, f"{hn}  {score}  {an}", color=TEXT_BRIGHT,
             fontsize=25, fontweight="bold")

    # ── Deserved result ──
    if abs(xh - xa) < 0.20:
        deserved = (f"Expected goals finished near-level ({xh:.2f} vs {xa:.2f}). On the "
        ...
        better = hn if xh > xa else an
        deserved = (f"{better} generated the better body of chances ({max(xh, xa):.2f} vs "
    ...
    finishing = (f"{hn} took {shh} shots ({oth} on target) and scored {gh} from {xh:.2f} xG — "
    ...
    if poss_leader == xt_leader:
        control = (f"{poss_leader} controlled the ball ({max(ph,pa):.0f}%) and converted that "
        ...
        control = (f"{poss_leader} saw more of the ball ({max(ph,pa):.0f}%), but it was "
    ...
    if pph is not None and ppa is not None:
        ...
        press = (f"Out of possession, {press_leader} pressed more aggressively "
                 f"(PPDA {min(pph,ppa):.1f} vs {max(pph,ppa):.1f}), engaging earlier and "
                 f"forcing the play back sooner.")
    else:
        press = (f"Both sides picked their pressing moments rather than committing to a "
    ...
    if pens is not None:
        ...
        decided = (f"The sides could not be separated, the scoreline reflecting how evenly the "
    ...
    right = [("CONTROL vs PENETRATION", control), ("THE PRESSING BATTLE", press),
             ("HOW IT WAS DECIDED", decided)]

    def _col(items, x, y0):
        y = y0
        for head, body in items:
            fig.text(x, y, head, color=C_GOLD, fontsize=10, fontweight="bold",
                     family="monospace")
            fig.text(x, y - 0.038, _wrap_text_simple(body, 52), color=TEXT_BRIGHT,
                     fontsize=11.5, va="top", linespacing=1.42)
    ...
    led_y = 0.135
    fig.add_artist(mpatches.Rectangle((0.06, led_y - 0.01), 0.88, 0.075,
                   transform=fig.transFigure, facecolor=BG_PANEL,
                   edgecolor=GRID_COL, lw=1.0, zorder=1))
    cols = [("xG", f"{xh:.2f}", f"{xa:.2f}"), ("SHOTS", str(shh), str(sha)),
            ("ON TARGET", str(oth), str(ota)), ("BIG CH.", str(bh), str(ba)),
            ("POSS %", f"{ph:.0f}", f"{pa:.0f}"), ("xT", f"{xth:.2f}", f"{xta:.2f}"),
            ("PPDA", f"{pph:.1f}" if pph else "—", f"{ppa:.1f}" if ppa else "—")]
    fig.text(0.075, led_y + 0.045, hn[:14], color=C_HOME, fontsize=8.5,
             fontweight="bold", family="monospace")
    fig.text(0.075, led_y + 0.012, an[:14], color=C_AWAY, fontsize=8.5,
             fontweight="bold", family="monospace")
    n = len(cols)
    x0, xw = 0.24, 0.70
    for i, (lbl, hv, av) in enumerate(cols):
        cx = x0 + (i + 0.5) * (xw / n)
        fig.text(cx, led_y + 0.052, lbl, ha="center", color=TEXT_DIM, fontsize=7.5,
                 fontweight="bold", family="monospace")
        fig.text(cx, led_y + 0.030, hv, ha="center", color=TEXT_BRIGHT, fontsize=11,
                 fontweight="bold", family="monospace")
        fig.text(cx, led_y + 0.008, av, ha="center", color=TEXT_BRIGHT, fontsize=11,
                 fontweight="bold", family="monospace")

    fig.text(0.06, 0.065, "End of report · Mostafa Saad", color=TEXT_DIM,
             fontsize=9.5, family="serif")
...
def _draw_glance_page(pdf, info, events, ppda, goals_df):
    ...
    sh = [0, 0]; sot = [0, 0]; xg = [0.0, 0.0]; poss = [50.0, 50.0]
    try:
        for j, tid in enumerate((hid, aid)):
            te = events[events["team_id"] == tid]
            if "is_shot" in te.columns:
    ...
    fig.text(0.5, 0.93, "MATCH AT A GLANCE", ha="center", color=TEXT_BRIGHT,
             fontsize=22, fontweight="bold",
             path_effects=[pe.withStroke(linewidth=3, foreground=BG_DARK)])
    fig.text(0.27, 0.85, hn, ha="right", color=C_HOME, fontsize=16, fontweight="bold")
    fig.text(0.50, 0.85, score, ha="center", color=TEXT_BRIGHT, fontsize=26, fontweight="bold")
    ...
    went_to_et, pens = _extra_time_and_pens(events, info)
    if pens is not None:
        fig.text(0.5, 0.815, f"AET · ({pens[0]} - {pens[1]} pens)",
                 ha="center", color=C_GOLD, fontsize=11, fontweight="bold")
    elif went_to_et:
        fig.text(0.5, 0.815, "AET (After Extra Time)",
                 ha="center", color=C_GOLD, fontsize=11, fontweight="bold")
    ...
    for i, (lbl, lv, hvv, avv, rv) in enumerate(rows):
        y = y0 - i * rh
        fig.text(0.5, y + 0.052, lbl, ha="center", color=TEXT_DIM,
        ...
        fig.text(0.18, y + 0.016, lv, ha="right", va="center",
                 color=C_HOME, fontsize=12, fontweight="bold")
        fig.text(0.82, y + 0.016, rv, ha="left", va="center",
                 color=C_AWAY, fontsize=12, fontweight="bold")
...
def _draw_toc_page(pdf, entries):
    ...
    fig = _new_dark_fig(11.7, 8.27)
    fig.text(0.08, 0.90, "CONTENTS", color=C_GOLD, fontsize=28, fontweight="bold",
             path_effects=[pe.withStroke(linewidth=3, foreground=BG_DARK)])
    fig.text(0.08, 0.855, "Match Analysis Report", color=TEXT_DIM,
    ...
    for title, page, col in entries:
        ...
        fig.text(0.90, y, str(page), color=TEXT_DIM, fontsize=12, va="center",
                 ha="right", family="monospace")
        fig.text(0.50, y, "." * 70, color=TEXT_FADED, fontsize=8,
                 va="center", ha="center")
        y -= 0.062
...
def _draw_glossary_page(pdf):
    ...
    import textwrap as _tw
    fig = _new_dark_fig(11.7, 8.27)
    fig.text(0.08, 0.92, "GLOSSARY & METHODOLOGY", color=C_GOLD,
             fontsize=23, fontweight="bold",
             path_effects=[pe.withStroke(linewidth=3, foreground=BG_DARK)])
    terms = [
        ("xG — Expected Goals", "Probability a shot becomes a goal (0–1), from location, angle, body part and assist type. Summed, it measures a team's chance quality."),
        ("xGoT — Expected Goals on Target", "xG recomputed from on-target shot placement — measures finishing quality after contact."),
        ("xT — Expected Threat", "Value added by moving the ball into more dangerous zones through passes and carries, before any shot is taken."),
        ("Big Chance", "A shot from a clear scoring situation — typically xG ≥ 0.30."),
        ("PPDA — Passes per Defensive Action", "Opponent passes allowed per defensive action; a lower number means more intense pressing."),
        ("Box Entry", "Ball entering the opponent's penalty area by pass or carry — a key penetration measure."),
        ("Zone 14", "Central zone just outside the box; the prime pre-assist / creative area."),
        ("Progressive Pass", "A pass that moves the ball significantly closer to goal, usually breaking a defensive line."),
        ("Pitch markers", "On shape maps, starters are drawn as circles and substitutes (bench players) as squares; node size = touches/passes."),
    ]
    y = 0.84
    for t, desc in terms:
        fig.text(0.08, y, t, color=TEXT_BRIGHT, fontsize=12.5, fontweight="bold")
        wrapped = "\n".join(_tw.wrap(desc, width=108))
        fig.text(0.08, y - 0.026, wrapped, color=TEXT_DIM, fontsize=10,
...
_COVER_BG_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                              "assets", "cover_bg_clean.png")
...
def _draw_match_summary_page(pdf, info, goals_df, ppda, events=None):
    ...
    # ── Eyebrow ─────────────────────────────────────────────────────────
    fig.text(0.5, 0.925, comp.upper() or "MATCH ANALYSIS REPORT", ha="center",
             color=C_GOLD, fontsize=12.5, fontweight="bold", family="monospace", zorder=5)
    fig.text(0.5, 0.893, " · ".join([b for b in [venue, mdate] if b]), ha="center",
             color=TEXT_DIM, fontsize=10.5, style="italic", zorder=5)
    ...
    fig.text(0.5, 0.70, score, ha="center", color=TEXT_BRIGHT, fontsize=72,
             fontweight="bold", va="center", **_st)
    _stn = dict(zorder=5, path_effects=[pe.withStroke(linewidth=3, foreground=BG_DARK)])
    fig.text(0.30, 0.70, home.upper(), ha="right", color=TEXT_BRIGHT, fontsize=19,
             fontweight="bold", va="center", **_stn)
    fig.text(0.70, 0.70, away.upper(), ha="left", color=TEXT_BRIGHT, fontsize=19,
             fontweight="bold", va="center", **_stn)
    ...
    if pens is not None:
        ...
        fig.text(wx, 0.648, "★ WINNERS", ha=("right" if winner == home else "left"),
                 color=C_GOLD, fontsize=9, fontweight="bold", family="monospace", zorder=5)
    elif went_to_et:
        ...
        hook = "Honours even"
    fig.text(0.5, 0.60, hook, ha="center", color=C_GOLD, fontsize=13.5,
             fontweight="bold", style="italic", zorder=5)

    # ── xG · Shots · Possession strip (gold leader, no colour bg) ──────
    if k is not None:
        strip = [("xG", f"{k['xg'][0]:.2f}", f"{k['xg'][1]:.2f}", k['xg'][0], k['xg'][1]),
                 ("Shots", str(k['shots'][0]), str(k['shots'][1]), k['shots'][0], k['shots'][1]),
                 ("Possession", f"{k['poss'][0]:.0f}%", f"{k['poss'][1]:.0f}%", k['poss'][0], k['poss'][1])]
        cw, gap = 0.235, 0.03
        total = len(strip) * cw + (len(strip) - 1) * gap
        x0 = (1 - total) / 2
        for i, (lbl, hv, av, hh, aa) in enumerate(strip):
            cx = x0 + i * (cw + gap)
            ax = fig.add_axes([cx, 0.20, cw, 0.15], zorder=5); ax.set_axis_off()
            ax.set_xlim(0, 1); ax.set_ylim(0, 1)
            ax.add_patch(mpatches.FancyBboxPatch((0.02, 0.02), 0.96, 0.96,
                         boxstyle="round,pad=0.0,rounding_size=0.05",
                         facecolor=BG_DARK, edgecolor=C_GOLD, lw=1.0, alpha=0.92))
            ax.text(0.5, 0.80, lbl.upper(), ha="center", color=TEXT_DIM,
                    fontsize=9, fontweight="bold", family="monospace")
            h_win = hh > aa if hh != aa else None
            ax.text(0.30, 0.40, hv, ha="center", color=C_GOLD if h_win is True else TEXT_BRIGHT,
                    fontsize=19, fontweight="bold", family="monospace")
            ax.text(0.70, 0.40, av, ha="center", color=C_GOLD if h_win is False else TEXT_BRIGHT,
                    fontsize=19, fontweight="bold", family="monospace")
            ax.text(0.30, 0.13, home[:3].upper(), ha="center", color=TEXT_DIM,
                    fontsize=7, fontweight="bold", family="monospace")
            ax.text(0.70, 0.13, away[:3].upper(), ha="center", color=TEXT_DIM,
                    fontsize=7, fontweight="bold", family="monospace")
    ...
    fig.text(0.5, 0.145,
             ...
             ha="center", color=TEXT_DIM, fontsize=10, zorder=5)
    fig.text(0.5, 0.075,
             "Executive Summary · The Match Story · Chance Creation · "
             "Build-up · Defence · The Verdict",
             ha="center", color=TEXT_FADED, fontsize=8.5, style="italic", zorder=5)
...
def _draw_goals_log_page(pdf, goals_df, info):
    ...
    for _, r in goals_df.iterrows():
        ...
        ax.text(0.98, cy, xg_txt,
                ha="right", va="center",
                color=_ui_text(C_GOLD), fontsize=9.5, transform=ax.transAxes)
...
def _shape_read(events, tid):
    ...
    try:
        ...
        if len(d) < 40:
            return ""
        mx = float(d["x"].mean()); sx = float(d["x"].std())
        sy = float(d["y"].std())
        yv = d["y"]
        left = float((yv >= 66.6).mean()); right = float((yv < 33.4).mean())
        height = ("high up the pitch" if mx >= 55 else
                  "deep in its own half" if mx <= 44 else "around the middle third")
        comp = ("vertically compact between its lines" if sx <= 17 else
                "stretched vertically" if sx >= 25 else "moderately spread from back to front")
...
def _ctx_for(events, info, ppda):
    ...
    hid, aid = info.get("home_id"), info.get("away_id")
    passes = [0, 0]; passpct = [0, 0]; box = [0, 0]
    topxt = [("", 0.0), ("", 0.0)]; xgsh = [0.0, 0.0]; otp = [0, 0]
    topcreator = [("", 0), ("", 0)]; topshooter = [("", 0, 0.0), ("", 0, 0.0)]
    topdef = [("", 0), ("", 0)]; clr = [0, 0]; tkl = [0, 0]; intc = [0, 0]
    rec = [0, 0]; keyp = [0, 0]
    ...
    try:
        for j, tid in enumerate((hid, aid)):
            ...
            tot = int(isp.sum()); comp = int((isp & (oc == "Successful")).sum())
            passes[j] = tot; passpct[j] = round(100 * comp / max(tot, 1))
            ...
            if issh.sum():
                sg = te[issh].groupby("player").agg(sh=("is_shot", "size"),
                                                    xg=("xG", "sum")) if "xG" in te else None
                if sg is not None and len(sg):
                    sg = sg.sort_values("xg", ascending=False)
                    topshooter[j] = (_last(sg.index[0]), int(sg.iloc[0]["sh"]), round(float(sg.iloc[0]["xg"]), 2))
            defmask = ty.isin(["Tackle", "Interception", "BallRecovery", "Clearance", "Block", "BlockedPass"])
            if defmask.sum():
    ...
    try:
        ...
        pass
    out.update({"passes": passes, "passpct": passpct, "box": box, "topxt": topxt,
                "xgsh": xgsh, "otp": otp, "topcreator": topcreator,
                "topshooter": topshooter, "topdef": topdef, "clr": clr, "tkl": tkl,
                "intc": intc, "rec": rec, "keyp": keyp, "formation": formation, "shape": shape,
                "hn": info.get("home_name") or "Home",
                "an": info.get("away_name") or "Away"})
    return out
...
def _analyst_commentary(fname, hn, an, ctx):
    ...
    gh, ga = col("goals"); xgh, xga = col("xg"); shh, sha = col("shots")
    oth, ota = col("sot"); bh, ba = col("big"); ph, pa = col("poss")
    xth, xta = col("xt"); pph, ppa = col("ppda"); pah, paa = col("passes")
    pcth, pcta = col("passpct"); bxh, bxa = col("box"); xsh_h, xsh_a = col("xgsh")
    oph, opa = col("otp")
    ...
    clr = col("clr"); tkl = col("tkl"); intc = col("intc"); rec = col("rec"); keyp = col("keyp")
    ...
    if "xg_flow" in f:
        lead = leader(xgh, xga); real = leader(gh, ga)
        sh_l = tsh_l[0] if (xgh >= xga) else tsh_l[1]
        who = (f"{lead} shaped the stronger body of chances ({max(xgh,xga):.2f} to {min(xgh,xga):.2f} xG)"
               if lead else f"the expected-goals split finished level ({xgh:.2f} to {xga:.2f})")
        star = f", led at the sharp end by {sh_l[0]} ({sh_l[1]} shots, {sh_l[2]:.2f} xG)" if sh_l and sh_l[0] else ""
        if lead and real and lead != real:
            flip = (f" Yet {real} carried the lead — a scoreline running ahead of the play, which almost always means a "
            ...
            flip = ""
        return ("How the danger actually accumulated",
    ...
    if "shot_map" in f and i is not None:
        sh, osh = s([shh, sha]); xg, oxg = s([xgh, xga]); big, obig = s([bh, ba])
        xps, _ = s([xsh_h, xsh_a]); otpc, _ = s([oph, opa]); shooter, _ = named(tsh_l)
        lead_by = f"{shooter[0]} the focal point ({shooter[1]} shots, {shooter[2]:.2f} xG)" if shooter and shooter[0] else "no single dominant shooter"
        verdict = ("worked the ball into clean, central positions" if xps >= 0.13 else
                   "was pushed onto lower-value efforts from distance or tight angles against a set defence")
        return (f"How {team} manufactured its shots",
    ...
    if "pass_network" in f and i is not None:
        pas, _ = s([pah, paa]); pct, _ = s([pcth, pcta]); pos, _ = s([ph, pa])
        ...
        hub = (f"{carrier[0]} its most threatening distributor ({carrier[1]:.2f} xT)"
               if carrier and carrier[0] else "no single dominant distributor")
        return (f"How {team} built and circulated",
    ...
    if ("xt_map" in f or "progressive" in f) and i is not None:
        xt, oxt = s([xth, xta]); carrier, _ = named(tx_l)
        share = f", {carrier[0]} carrying the most of it ({carrier[1]:.2f})" if carrier and carrier[0] else ""
        eff = "far more" if oxt and xt > oxt * 1.4 else ("more" if xt >= oxt else "less")
        conc = ("a team leaning on one progressor to unlock the block" if (carrier and carrier[1] and xt and carrier[1] > 0.2 * xt)
                else "threat shared across several carriers")
        return (f"Who moved {team} into danger",
    ...
    if any(x in f for x in ("danger", "box_entries", "zone14")) and i is not None:
        box_, _ = s([bxh, bxa]); big, obig = s([bh, ba]); kp, _ = s(keyp)
        creator, _ = named(tc_l)
        lead_by = f"{creator[0]} leading the creation ({creator[1]} key passes)" if creator and creator[0] else "no single dominant creator"
        return (f"How {team} turned territory into openings",
    ...
    if "defensive" in f and i is not None:
        cl, _ = s(clr); it, _ = s(intc); tk, _ = s(tkl); defender, _ = named(td_l)
        led = f"{defender[0]} carrying the load ({defender[1]} actions)" if defender and defender[0] else "the workload spread across the unit"
        nature = ("a block clearing its lines rather than winning the ball to keep it — and against a counter-pressing "
                  "opponent, clearing without retaining tends to invite the very next wave"
                  if cl >= max(it + tk, 1) else
                  "a side winning the ball to keep it rather than simply clearing, the sign of front-foot, proactive defending")
        return (f"How {team} defended, and where",
    ...
    if "ppda" in f or "high_turnover" in f or "turnover" in f:
        ...
        pline = (f"{press} pressed the more aggressively (PPDA {min(pph,ppa):.1f} to {max(pph,ppa):.1f}), engaging after "
                 f"fewer opponent passes. ") if (pph and ppa and press) else ""
        return ("The pressing battle, and what it produced",
    ...
    if any(x in f for x in ("territor", "possession", "ball_touches", "dominating")):
        lead = leader(ph, pa); xlead = leader(xth, xta)
        pl = (f"{lead} owned the ball ({max(ph,pa):.0f}% to {min(ph,pa):.0f}%)") if lead else \
             f"possession finished near-level ({ph:.0f}% to {pa:.0f}%)"
        tie = (f" And it translated: {xlead} also led the ball-progression value ({max(xth,xta):.2f} to {min(xth,xta):.2f} xT)."
               if (lead and xlead and lead == xlead) else
               (f" But it did not fully translate — {xlead} generated the greater threat ({max(xth,xta):.2f} to "
                ...
                if (lead and xlead and lead != xlead) else ""))
        return ("Who owned the space, and where it mattered",
    ...
    if "match_stats" in f:
        xlead = leader(xgh, xga); plead = leader(ph, pa)
        tension = ("point the same way — a reliable, one-directional story"
                   if (xlead and plead and xlead == plead) else
                   "pull apart: possession one way, chance quality the other — the classic signature of control without "
                   "penetration, the ball and the space owned by a team that could not turn either into the cleaner openings")
        return ("The control panel — reading the numbers together",
    ...
    if "xt_per_minute" in f or "xt_minute" in f:
        return ("Where the momentum actually swung",
...
def _pdf_page_with_commentary(pdf, fig, heading: str, body: str):
    ...
    new_fig.text(
        0.93, 0.055, "What does this visual add?",
...
def _draw_team_stats_compare_page(pdf, info, events, ppda):
    ...
    def _shots_for(team_id) -> int:
        ...
        try:
            s = events[(events["team_id"] == team_id) &
                       (events["is_shot"].fillna(False) == True)]  # noqa: E712
    ...
    _foul_rows = events[events["type"] == "Foul"] if (
        events is not None and not events.empty and "type" in events.columns) else None
    _splits_fouls = (_foul_rows is not None and "outcome" in events.columns and
    ...
    def _duels(team_id):
        ...
        to_self = _c(tm, "TakeOn"); to_opp = _c(om, "TakeOn")
        gt = to_self + to_opp
    ...
    defensive_rows = [
        ...
        ("Fouls",
         _fouls_committed(home_id),
         _fouls_committed(away_id)),
    ...
    def _draw_stats_panel(panel_xywh, title, rows_list, accent, home_color, away_color):
        ...
        ax.add_patch(mpatches.Rectangle((0, 0.93), 1, 0.07,
                                         facecolor=BG_HEADER, lw=0,
                                         transform=ax.transAxes))
        ax.add_patch(mpatches.Circle((0.018, 0.965), 0.011,
                                     facecolor=accent, edgecolor="none",
                                     transform=ax.transAxes))
        ax.text(0.04, 0.965, title.upper(), ha="left", va="center",
                color=TEXT_BRIGHT, fontsize=10.5, fontweight="bold",
                family=FONT_MONO, transform=ax.transAxes)

        # ── Team labels row (just below the title strip) ──
        ax.text(0.02, 0.885, hn, ha="left", va="center",
                color=home_color, fontsize=9.5, fontweight="bold",
                family=FONT_MONO, transform=ax.transAxes)
        ax.text(0.98, 0.885, an, ha="right", va="center",
                color=away_color, fontsize=9.5, fontweight="bold",
                family=FONT_MONO, transform=ax.transAxes)
        # subtle divider under team labels
        ax.plot([0.02, 0.98], [0.85, 0.85], color=GRID_COL, lw=1.0,
                transform=ax.transAxes)
        ...
        for i, (label, hv, av) in enumerate(rows_list):
            ...
            if h_ratio:
                ...
                ax.add_patch(mpatches.Rectangle(
                    ...
                    facecolor=home_color, alpha=0.88, lw=0,
                    transform=ax.transAxes))
            ...
            if a_ratio:
                ...
                ax.add_patch(mpatches.Rectangle(
                    ...
                    facecolor=away_color, alpha=0.88, lw=0,
                    transform=ax.transAxes))
            ...
            ax.text(_hx, cy, h_disp, ha="right", va="center",
                    color=leader_home_col,
                    fontsize=_vfs, fontweight="bold", family=FONT_MONO,
                    transform=ax.transAxes)
            ax.text(_ax, cy, a_disp, ha="left", va="center",
                    color=leader_away_col,
                    fontsize=_vfs, fontweight="bold", family=FONT_MONO,
                    transform=ax.transAxes)
            # Stat label — centered on a chip, never over the bars.
            ax.text(0.50, cy, label, ha="center", va="center",
                    color=TEXT_DIM, fontsize=8.7, fontweight="bold",
    ...
    _draw_stats_panel((0.035, 0.50, 0.40, 0.34),
                      "Attack & Passing", attack_rows, home_col, home_col, away_col)
    _draw_stats_panel((0.035, 0.06, 0.40, 0.40),
                      "Defensive Actions", defensive_rows, away_col, home_col, away_col)
    ...
    if pens is not None:
        ...
        result_line = ""
    if result_line:
        fig.text(0.235, 0.862, result_line, ha="center", va="center",
    ...
    def _mini_dial(ax_pos, name, value, color):
        ...
        fig.text(cx, ax_pos[1] + ax_pos[3] + 0.012, name,
                 ha="center", color=TEXT_BRIGHT, fontsize=10.5,
                 fontweight="bold", family=FONT_SANS)
        val_str = f"{value:.2f}" if value is not None else "N/A"
        fig.text(cx, ax_pos[1] - 0.008, val_str, ha="center", color=color,
                 fontsize=15, fontweight="bold", family=FONT_MONO)
        lbl, lcol = _ppda_intensity_label(value)
        fig.text(cx, ax_pos[1] - 0.032, lbl, ha="center", color=lcol,
    ...
    fig.add_artist(mpatches.Circle((0.475, 0.862), 0.0035,
                   transform=fig.transFigure, facecolor=C_GOLD,
                   edgecolor="none", zorder=2))
    fig.text(0.487, 0.862, "PRESSING · PPDA",
             ha="left", color=TEXT_BRIGHT, fontsize=10.5, fontweight="bold",
             family=FONT_MONO)
    fig.text(0.717, 0.828,
             "Passes per Defensive Action — lower = more aggressive press",
             ha="center", color=TEXT_DIM, fontsize=8.5, family=FONT_SANS)
    ...
    if h_ppda is not None and a_ppda is not None:
        if h_ppda < a_ppda:
            verdict = f"{hn} pressed more aggressively"; vcol = home_text_col
            diff = a_ppda - h_ppda
        elif a_ppda < h_ppda:
            verdict = f"{an} pressed more aggressively"; vcol = away_text_col
            diff = h_ppda - a_ppda
        else:
            verdict = "Both teams pressed equally"; vcol = TEXT_BRIGHT
            diff = 0.0
        fig.text(0.717, 0.585, verdict, ha="center", color=vcol,
                 fontsize=12, fontweight="bold")
        if diff > 0:
            fig.text(0.717, 0.562, f"PPDA differential: {diff:.2f}",
    ...
    for s in com_ax.spines.values():
        s.set_edgecolor(GRID_COL); s.set_linewidth(1.0)
    ...
    com_ax.add_patch(mpatches.Rectangle((0, 0.90), 1, 0.10,
                                         facecolor=BG_HEADER,
                                         transform=com_ax.transAxes, lw=0))
    com_ax.add_patch(mpatches.Circle((0.022, 0.95), 0.011,
                                     facecolor=C_GOLD, edgecolor="none",
                                     transform=com_ax.transAxes))
    com_ax.text(0.045, 0.95, "READING THIS PAGE",
                ha="left", va="center", color=TEXT_BRIGHT,
                fontsize=9.5, fontweight="bold", family=FONT_MONO,
                transform=com_ax.transAxes)
    com_ax.plot([0, 1], [0.90, 0.90], color=GRID_COL, lw=1.0,
                transform=com_ax.transAxes)
    ...
    for (color, heading, body), cx in zip(sections, col_cx):
        com_ax.add_patch(mpatches.Circle((cx - 0.145, 0.82), 0.010,
                                          facecolor=color, lw=0,
                                          transform=com_ax.transAxes,
                                          zorder=4))
        com_ax.text(cx - 0.125, 0.82, heading, ha="left", va="center",
                    color=color, fontsize=8.3, fontweight="bold",
                    transform=com_ax.transAxes,
                    path_effects=[pe.withStroke(linewidth=2,
                                                foreground=BG_DARK)])
        wrapped = textwrap.fill(body, width=30)
        com_ax.text(cx - 0.145, 0.74, wrapped, ha="left", va="top",
                    color=TEXT_MAIN, fontsize=7.8,
                    transform=com_ax.transAxes, linespacing=1.5)
...
def run_analysis(match_data: dict,
    ...
    نقطة الدخول الموحدة. بتطلع PDF واحد منظم بالـ dark theme:
        ...
        5. Embedded extra_figs (الـ 39 visual الأصلية لو اتبعتت)
        6. (اختياري) دمج مع PDFs الأصلية في merge_with_pdfs
    ...
    وكل visual وملف PDF النهائي بيتحفظوا في فولدر المباراة نفسه.
    """
    ...
    def _visual_phase(fname: str) -> int:
        f = (fname or "").lower()
        if any(k in f for k in ("xg_flow", "breakdown_goals", "shot_comparison",
                                "xg_tiles", "xg_summary", "gk_saves",
                                "xt_per_minute", "match_stats")):
        ...
        if any(k in f for k in ("pass_network", "xt_map", "pass_thirds",
                                "progressive", "avg_position", "pass_target")):
            return 4  # build-up & progression
        if any(k in f for k in ("defensive_hm", "defensive_summary",
                                "high_turnovers", "ppda")):
    ...
    _next_of = {fn: (_flat_order[i + 1] if i + 1 < len(_flat_order) else None)
                for i, fn in enumerate(_flat_order)}

    def _bridge(next_fname):
        if not next_fname:
            return ("This is the final analytical page — read it back against the whole "
        ...
        for key, why in (
            ...
            ("breakdown_goals", "to separate shot volume from finishing and see how the goals arrived"),
            ("shot_map", "to check whether this turned into clean shooting positions"),
            ("shot_comparison", "to weigh shot quantity against genuine chance quality"),
            ("xg_tiles", "to compare chance quality before the shot with quality after contact"),
            ("xg_summary", "to compare chance quality before the shot with quality after contact"),
            ...
            try:
                ...
                for side in ("home", "away"):
                    ...
                    for player, pfig, prole, note in radar_data.get(side, {}).get("figs", []):
                        pg += 1
                        _draw_player_radar_page(pdf, player, pfig, accent,
                                                page_no=pg, team_name=tname,
                                                role=prole, commentary=note)
            except Exception:
                pass
